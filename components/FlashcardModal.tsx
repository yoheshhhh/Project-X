...
export function FlashcardModal({ open, cards, onClose }: { open: boolean; cards: Flashcard[]; onClose: () => void }) {
  const [index, setIndex] = useState(0);
  const [flipped, setFlipped] = useState(false);

  useEffect(() => {
    if (open) { setIndex(0); setFlipped(false); }
  }, [open, cards]);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
      <div className="bg-gray-800 rounded-xl shadow-xl max-w-lg w-full p-6">
        ...
        <div className="mt-4 flex justify-between">
          <span className="text-sm text-gray-400">{cards.length > 0 ? `Card ${index + 1} of ${cards.length}` : 'No cards'}</span>
          <button type="button" onClick={next} className="py-2 px-4 rounded-lg bg-blue-600 hover:bg-blue-700">{isLast ? 'Continue to retry quiz' : 'Next'}</button>
        </div>
      </div>
    </div>
  );
