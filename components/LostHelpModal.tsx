...
export function LostHelpModal({ open, content, onClose }: { open: boolean; content: LostHelpResponse | null; onClose: () => void }) {
  if (!open) return null;
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
      <div className="bg-gray-800 rounded-xl shadow-xl max-w-lg w-full max-h-[90vh] overflow-y-auto p-6">
        ...
        <button type="button" onClick={onClose} className="mt-6 w-full py-2 rounded-lg bg-gray-600 hover:bg-gray-500">Close</button>
      </div>
    </div>
  );
}
