...
export default function WatchPage() {
  ...
  // Use manual segments from config if present; otherwise build from duration
  useEffect(() => {
    if (demoModule.segments?.length) {
      setSegments(demoModule.segments);
    }
  }, []);
  ...
  return (
    <div className="max-w-4xl mx-auto p-4 md:p-6">
      <h1 className="text-2xl font-bold mb-2">Content Engine Demo</h1>
          ...
          return (
            <button
              ...
              className={`px-3 py-2 rounded-lg text-sm font-medium transition-colors inline-flex items-center gap-1.5 ${
                canOpen ? 'bg-gray-600 hover:bg-gray-500 text-white' : 'bg-gray-700 text-gray-500 cursor-not-allowed'
              }`}
      ...
      <div className="relative">
        ...
        <button
          ...
          className="mt-3 px-4 py-2 rounded-lg bg-amber-600 hover:bg-amber-700 text-sm font-medium"
        >
          I&apos;m Lost
        </button>
      </div>
