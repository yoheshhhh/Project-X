/**
 * Progress service – localStorage for now; swap to Firestore when Narhen's side is ready.
 */
...
export const progressService: ProgressService = {
  ...
  async setProgress(progress: ModuleProgress): Promise<void> {
  ...
  async recordQuizResult(
    ...
    const attempts = current.segmentAttempts[segmentIndex] ?? 0;
    current.segmentAttempts[segmentIndex] = attempts + 1;
    current.quizScores[segmentIndex] = score;
    current.segmentMastery[segmentIndex] = getScoreMastery(score);
    if (score >= 70) {
      current.unlockedSegmentIndex = Math.max(
        current.unlockedSegmentIndex,
        segmentIndex + 1
      );
    }
    all[key] = current;
  ...
  async recordQuizAttempt(
    ...
    current.segmentAttempts[segmentIndex] =
      (current.segmentAttempts[segmentIndex] ?? 0) + 1;
    all[key] = current;
    saveAll(all);
  },

  async recordSegmentReached(
    userId: string,
    moduleId: string,
    segmentIndex: number
  ): Promise<void> {
