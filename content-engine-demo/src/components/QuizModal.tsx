...
export function QuizModal({
  ...
  const handleSubmit = () => {
    ...
    if (pct >= PASS_THRESHOLD) {
      // Show Proceed button; don't call onPass yet
    } else {
      onFail();
    }
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
      <div className="bg-gray-800 rounded-xl shadow-xl max-w-lg w-full max-h-[90vh] overflow-y-auto p-6">
        <h2 className="text-xl font-semibold mb-4">
          Segment {segmentIndex + 1} – Check your understanding
        </h2>
                ...
                <div key={q.id}>
                  ...
                  <div className="space-y-2">
                    {q.options.map((opt, i) => (
                      <label
                        key={i}
                        className="flex items-center gap-2 cursor-pointer p-2 rounded bg-gray-700 hover:bg-gray-600"
                      >
                        <input
                          type="radio"
                          name={q.id}
